...
var personSchema = new Schema({
  ...
  comments: {type: [Array], default: ["no comments"]},
  registered: Boolean,
  personal_id: {type: Number, unique: false}
  })
personSchema.methods.speak = ()=> `Hi, my name is ${this.name}!`; //a method that will belong to the Person model
...
// follows Node convention
const createAndSavePerson = function(done) {
  const dude = new Person({name: "Xo", age: 63, favoriteFoods: ["sandwiches"]});
  dude.save((err, data) => {
    if(err) {
      done(err);
    }
...
const createManyPeople = function(arrayOfPeople, done) {
  Person.create(arrayOfPeople, function(error, data) {if(error) {done(error)} else done(null, data)});
};
...
var findPeopleByName = function(personName, done) {
  Person.find({name: personName, registered: {$in: [true, false, null]}, age: {$gte: 0}}, (err, data)=> (err)? done(err):done(null, data))
...
//  Person.findOne({name: personName}, function(error, data){
//  if(error) {
//    done(error)}
//  else {
//    data.age= ageToSet;
...
var removeById = function(personId, done) {
  ...
  Person.findByIdAndRemove(personId, (err, removedData)=>(err)?done(err):done(null,removedData));
...
var removeManyPeople = function(done) {
  ...
  Person.remove({name: nameToRemove}, (err, removalSummary)=>(err)?done(err):done(null, removalSummary))
};
...
var queryChain = function(done) {
  ...
  Person.find({favoriteFoods: foodToSearch}).sort({name: 1}).limit(2).select("name favoriteFoods").exec((err, data)=>(err)?done(err):done(null,data))
};
